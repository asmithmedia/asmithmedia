...
const PLAN_MAP: Record<string, string> = {
  [process.env.SITEVITALS_SO_PRICE_SINGLE || "price_1THyiMHSMVzgSAR23KusdlTa"]: "single",
  [process.env.SITEVITALS_SO_PRICE_UNLIMITED || "price_1THyiuHSMVzgSAR2WlPZ1BPY"]: "unlimited",
};

const PRO_ZIP_URL = "https://asmith.media/sitevitals/speed-optimizer/downloads/sitevitals-speed-optimizer-pro.zip";
...
<tr><td style="background:linear-gradient(135deg,#1e293b 0%,#0f172a 100%);padding:24px 32px;text-align:center;">
    <h1 style="color:#ffffff;margin:0;font-size:22px;">Welcome to Speed Optimizer Pro!</h1>
    <p style="color:#94a3b8;margin:8px 0 0;font-size:14px;">Your site is about to get a lot faster.</p>
</td></tr>
...
<tr><td style="padding:32px;">
    ...
    <p style="margin:0 0 16px;">
        <a href="${downloadUrl}" style="display:inline-block;padding:12px 24px;background:#6366f1;color:#ffffff;text-decoration:none;border-radius:6px;font-weight:bold;font-size:14px;">
            Download sitevitals-speed-optimizer-pro.zip
        </a>
    </p>
    ...
    <ol style="color:#475569;font-size:14px;line-height:1.8;padding-left:20px;">
        <li>Install the <strong>free SiteVitals Speed Optimizer</strong> plugin from WordPress.org (if not already installed).</li>
        ...
        <li>Go to <strong>Speed Optimizer &gt; Settings &gt; Pro License</strong>.</li>
        <li>Paste your license key and click <strong>Activate</strong>.</li>
    </ol>

    <h2 style="font-size:16px;color:#1e293b;margin:24px 0 8px;">What&rsquo;s Included</h2>
    <table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;color:#475569;">
        <tr><td style="padding:6px 0;">&#10003; Critical CSS generation</td><td style="padding:6px 0;">&#10003; Unused CSS removal</td></tr>
        <tr><td style="padding:6px 0;">&#10003; WebP/AVIF conversion</td><td style="padding:6px 0;">&#10003; Advanced JS delay</td></tr>
        <tr><td style="padding:6px 0;">&#10003; CDN integration</td><td style="padding:6px 0;">&#10003; Preload &amp; prefetch</td></tr>
        <tr><td style="padding:6px 0;">&#10003; Performance history</td><td style="padding:6px 0;">&#10003; White-label reports</td></tr>
    </table>
</td></tr>

<tr><td style="padding:16px 32px;background:#f8fafc;text-align:center;border-top:1px solid #e5e7eb;">
    <p style="color:#9ca3af;font-size:12px;margin:0;">
        Need help? Contact <a href="mailto:sarah@asmith.media" style="color:#6366f1;">sarah@asmith.media</a>
    </p>
    <p style="color:#9ca3af;font-size:12px;margin:4px 0 0;">
        &copy; ${new Date().getFullYear()} SiteVitals by A. Smith Media. All rights reserved.
    </p>
</td></tr>
...
export async function POST(req: NextRequest) {
  const webhookSecret = process.env.SITEVITALS_SO_STRIPE_WEBHOOK_SECRET;
  const stripeKey = process.env.STRIPE_SECRET_KEY;
  const privateKeyHex = process.env.SITEVITALS_SO_ED25519_PRIVATE_KEY;
  const resendApiKey = process.env.RESEND_API_KEY;

  if (!webhookSecret || !stripeKey || !privateKeyHex || !resendApiKey) {
    console.error("SiteVitals SO webhook: missing env vars");
  ...
  if (!event) {
    console.error("SiteVitals SO webhook: invalid signature");
  ...
  if (!customerEmail) {
    console.error("SiteVitals SO webhook: no customer email");
  ...
  if (!licenseKey) {
    console.error(`SiteVitals SO webhook: license generation failed for ${customerEmail}`);
  ...
  try {
    await resend.emails.send({
      ...
      to: customerEmail,
      subject: `Your SiteVitals Speed Optimizer Pro License Key (${planLabel})`,
      html: buildLicenseEmailHtml(licenseKey, planLabel, expires, PRO_ZIP_URL),
    });

    console.log(`SiteVitals SO: license delivered to ${customerEmail} (plan: ${plan})`);
  } catch (err) {
    console.error(`SiteVitals SO: email failed for ${customerEmail}`, err);
  }
